...
const CompleteProfile = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyBs_YzTSnimtfiEbkXFfeyjVVWYrCwWaXg",
      {
        method: "POST",
        body: JSON.stringify({
  ...
  const submitHandler = (e) => {
    ...
    if (enteredName === "" || enteredPhotoUrl === "") {
      ...
      return;
        ...
        fetch(
          "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyBs_YzTSnimtfiEbkXFfeyjVVWYrCwWaXg",
          {
            method: "POST",
            body: JSON.stringify({
              idToken: localStorage.getItem("token"),
